...
# Clean old/stale assets if present (e.g. model bin is now downloaded on-demand)
for stale in [
    ASSETS / "dex" / "mediapipe-runtime.dex",
    ASSETS / "jni" / "arm64-v8a" / "libmediapipe_tasks_vision_jni.so",
    ASSETS / "model" / "head_det.bin",
]:
    if stale.exists():
        stale.unlink()
...
local_bin = ROOT / "models" / "head_det.bin"
bin_hash = sha256(local_bin) if local_bin.is_file() else "df250b1cc91b56e90af9e62b7a1decba35e17a30d4129df49aea161c21444fea"
bin_size = local_bin.stat().st_size if local_bin.is_file() else 6047932
